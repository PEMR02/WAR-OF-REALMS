...
namespace Project.Gameplay.Map
{
    ...
    public static class MapGenConfigFactory
    {
        public static MapGenConfig CreateFrom(MatchConfig match)
        {
            ...
            if (c.lakeCount > 0)
                c.maxLakeCells = Mathf.Max(c.maxLakeCells, c.riverCount > 0 ? 480 : 320);
            ...
            c.debugTerrainSplatOverlayInPlay = false;
            ...
            c.waterVisualPipeline = match.water.visualPipeline;
            c.riverWaterMaterial = match.water.material;
            c.riverWaterMaterialMode = match.water.materialMode;
            if (match.water.riverMaterial != null)
            {
                c.riverWaterMaterial = match.water.riverMaterial;
                c.riverWaterMaterialMode = match.water.riverMaterialMode;
            }
            c.tributaryWaterMaterial = match.water.tributaryMaterial;
            c.tributaryWaterMaterialMode = match.water.tributaryMaterial != null
                ? match.water.tributaryMaterialMode
                : c.riverWaterMaterialMode;
            c.lakeWaterMaterial = match.water.lakeMaterial;
            c.lakeWaterMaterialMode = match.water.lakeMaterialMode;
            c.seaWaterMaterial = match.water.seaMaterial;
            c.seaWaterMaterialMode = match.water.seaMaterialMode;
            c.riverWaterAlpha = match.water.alpha;
