...
namespace Project.Gameplay.Map.Generation
{
    ...
    public static class MatchConfigCompiler
    {
            ...
            bool logSummary = true)
        {
            ...
            if (cfg != null && result.RuntimeContext != null && result.RuntimeContext.uwpHydrologyProfileWasAppliedToCompiledConfig)
            {
                UnityEngine.Debug.LogWarning(
                    $"[HydrologyCompileAudit] Post-compile MapGen: riverCount={cfg.riverCount} lakeCount={cfg.lakeCount} " +
                    $"seed={cfg.seed} maxAttempts={cfg.maxTotalRiverBuildAttempts} uwpOwned={cfg.uwpOwnedVisualPolicy}");
            }
        ...
        public static bool ApplyLegacyResourceFallbackFromScene(ResourceRuntimeSettings res, Component sceneHost)
        {
            return false;
        }
        ...
        static void CopyDebugFlagsFromTemplate(MapGenConfig target, MapGenConfig template)
        {
            ...
            target.debugTerrainSplatOverlayInPlay = template.debugTerrainSplatOverlayInPlay;
            if (!target.uwpOwnedVisualPolicy)
            {
        ...
        public static void ApplyMatchToMapGen(MatchConfig match, MapGenConfig config, RuntimeMapGenerationSettings diagnostics = null)
        {
            ...
            if (match.water.overrideVisualPipeline)
                config.waterVisualPipeline = match.water.visualPipeline;
            if (config.riverWaterMaterial == null && match.water.material != null)
            {
                config.riverWaterMaterial = match.water.material;
                config.riverWaterMaterialMode = match.water.materialMode;
            }
            if (match.water.riverMaterial != null)
            {
                config.riverWaterMaterial = match.water.riverMaterial;
                config.riverWaterMaterialMode = match.water.riverMaterialMode;
            }
            if (match.water.tributaryMaterial != null)
            {
                config.tributaryWaterMaterial = match.water.tributaryMaterial;
                config.tributaryWaterMaterialMode = match.water.tributaryMaterialMode;
            }
            if (match.water.lakeMaterial != null)
            {
                config.lakeWaterMaterial = match.water.lakeMaterial;
                config.lakeWaterMaterialMode = match.water.lakeMaterialMode;
            }
            if (match.water.seaMaterial != null)
            {
                config.seaWaterMaterial = match.water.seaMaterial;
                config.seaWaterMaterialMode = match.water.seaMaterialMode;
            }
            config.riverWaterAlpha = match.water.alpha;
