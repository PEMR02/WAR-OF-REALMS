"""
WAR OF REALMS — Tree Generator v2 (MEJORADO)
=============================================
Cambios respecto a v1:
  - LOD2 factor subido de 0.30 → 0.45 (ya no se ven pelados)
  - LOD1 factor subido de 0.60 → 0.68 (más hojas en rango medio)
  - Mínimo de hojas por cluster: 8 (antes 5) → nunca quedan ramas desnudas
  - Color de hoja diferente por variante (5 materiales)
  - TallForest: más ramas altas (10→12), ángulo superior más abierto
  - SparseNatural: leaves_range subido (26-38 → 32-46) para que tenga más cuerpo
  - WideOak: tronco más grueso (R 0.36→0.40), hojas más grandes (ls 0.43→0.46)
  - YoungDense: corona más compacta (cr 2.6→2.4), más ramas medias (9→10)

Pegar completo en Blender MCP → execute_blender_code
"""
...
b_noise.inputs["Scale"].default_value=14; b_noise.inputs["Detail"].default_value=8
...
# ── HOJAS — 5 materiales con colores distintos ────────────────────────────────
def _leaf_mat(name, rgb, rough=0.72, sss=0.08):
    mat = bpy.data.materials.new(name)
    mat.use_nodes = True; ln = mat.node_tree; ln.nodes.clear()
    bsdf = ln.nodes.new("ShaderNodeBsdfPrincipled"); bsdf.location=(400,0)
    bsdf.inputs["Base Color"].default_value = (*rgb, 1)
    bsdf.inputs["Roughness"].default_value  = rough
    try: bsdf.inputs["Subsurface Weight"].default_value = sss
    except: pass
    out = ln.nodes.new("ShaderNodeOutputMaterial"); out.location=(700,0)
    ln.links.new(bsdf.outputs["BSDF"], out.inputs["Surface"])
    mat.use_backface_culling = False
    return mat

LEAF_MATS = {
    "TallForest":    _leaf_mat("MAT_Leaf_Tall",    (0.10, 0.40, 0.06), rough=0.70, sss=0.07),
    "WideOak":       _leaf_mat("MAT_Leaf_Oak",     (0.05, 0.26, 0.03), rough=0.76, sss=0.09),
    "YoungDense":    _leaf_mat("MAT_Leaf_Young",   (0.14, 0.44, 0.08), rough=0.65, sss=0.10),
    "WindBent":      _leaf_mat("MAT_Leaf_Wind",    (0.07, 0.31, 0.04), rough=0.73, sss=0.07),
    "SparseNatural": _leaf_mat("MAT_Leaf_Sparse",  (0.10, 0.30, 0.03), rough=0.82, sss=0.05),
}
...
def make_tree(cfg, lf, pos, bmat, lmat):
    ...
    def _branch(p0, d_, length, radius, level):
        ...
        if level==3:
            nl = max(8, int(rng.randint(lmin,lmax)*lf))   # min 8 hojas (v2: era 5)
            cr__ = rng.uniform(0.20, 0.30)
            _cluster(p1, nl, cr__)
            return
    ...
    for pi in range(mn):
        ...
        _branch(Vector((ox,0,z0)), pd,
                cr*rng.uniform(mid["l_f"]-0.08, mid["l_f"]+0.08),
    ...
    print(f"  {tag:<34} | lf={lf:.2f} | {len(me.polygons):>7,} polys | ~{lp//8:>5,} hojas")
    return obj

# ── CONFIGURACIONES v2 ────────────────────────────────────────────────────────
TREES = [
    {
        "tag": "TallForest", "H":9.5, "R":0.22, "cz":5.5, "cr":2.8, "seed":101, "ls":0.34,
        "leaves_range": (38, 52),
        "mid": {"n":5,  "elev":(44,60), "z":(0.55,0.80), "l_f":0.44, "r_f":0.40},
        "top": {"n":12, "elev":(8,24),  "z":(0.72,0.97), "l":(2.2,3.6), "r_f":0.30},
    },
    {
        "tag": "WideOak", "H":7.5, "R":0.40, "cz":3.8, "cr":4.8, "seed":202, "ls":0.46,
        "leaves_range": (44, 62),
        "mid": {"n":8,  "elev":(50,72), "z":(0.40,0.76), "l_f":0.55, "r_f":0.50},
        "top": {"n":8,  "elev":(16,38), "z":(0.65,0.92), "l":(3.0,4.5), "r_f":0.38},
    },
    {
        "tag": "YoungDense", "H":6.5, "R":0.20, "cz":2.6, "cr":2.4, "seed":303, "ls":0.31,
        "leaves_range": (42, 56),
        "mid": {"n":10, "elev":(35,58), "z":(0.35,0.76), "l_f":0.46, "r_f":0.38},
        "top": {"n":11, "elev":(10,28), "z":(0.62,0.95), "l":(1.8,3.0), "r_f":0.28},
    },
    {
        "tag": "WindBent", "H":8.0, "R":0.27, "cz":4.2, "cr":3.6, "seed":404, "ls":0.38,
        "leaves_range": (38, 52), "lean_x": 1.4,
        "mid": {"n":7,  "elev":(38,58), "z":(0.44,0.78), "l_f":0.50, "r_f":0.44},
        "top": {"n":8,  "elev":(12,30), "z":(0.68,0.94), "l":(2.5,4.0), "r_f":0.34},
    },
    {
        "tag": "SparseNatural", "H":8.8, "R":0.24, "cz":4.8, "cr":3.4, "seed":505, "ls":0.37,
        "leaves_range": (32, 46),
...
# LOD factors mejorados: LOD2 ya no queda pelado
LODs = [(1.0,"LOD0"), (0.68,"LOD1"), (0.45,"LOD2")]

print("Generando 15 arboles v2...")
print(f"  {chr(34)}Nombre{chr(34):<34} | lf   | Polys   | Hojas")
print("  " + "-"*68)

for ti, cfg in enumerate(TREES):
    base_tag = cfg["tag"]
    lmat = LEAF_MATS[base_tag]
    for li, (lf, ltag) in enumerate(LODs):
        full_cfg = {**cfg, "tag": f"Tree_{base_tag}_{ltag}"}
        make_tree(full_cfg, lf, (ti*15.0, li*18.0, 0), bark_mat, lmat)

print("\\n=== 15 ARBOLES v2 COMPLETADOS ===")
'''
