...
namespace Project.Gameplay.Map.Generator
{
    ...
    public static partial class RiverRouteGenerator
    {
        ...
        public static void PreparePlannedLakeSinkCandidates(GridSystem grid, MapGenConfig config, IRng rng)
        {
            ...
            int w = grid.Width;
            int h = grid.Height;
            int minSep = Mathf.Max(10, Mathf.Min(w, h) / 10);
            ...
            out string rejectReason)
        {
            ...
            {
                int tries = pat == RiverMainPattern.HighlandToLake || pat == RiverMainPattern.BorderToLake ? 22 : 16;
            ...
            out string rejectReason)
        {
            ...
            for (int pair = 0; pair < RandomAnchorPairsMax; pair++)
            {
                ...
                if (!TryPickBorderLandAnchor(grid, w, h, rng, edgeA, cornerExcluded, edgeInset, false, out Vector2Int start))
                    continue;
                if (!TryPickBorderLandAnchor(grid, w, h, rng, edgeB, cornerExcluded, edgeInset, false, out Vector2Int goal))
                    continue;
